...
static void fallback_provider_vma(struct vma_area *vma)
{
	vma->e->flags |= MAP_ANONYMOUS;
	vma->e->status &= ~VMA_EXT_PROVIDER;
}
...
/*
 * Return 1 if the provider handled the VMA, 0 for normal handling, and -1
 * on error.
 */
static int open_provider_vma(struct vma_area *vma, int pid, unsigned int vma_id,
			     bool *fd_acquired)
{
	int fd = -1;
	int ret;
	...
	ret = extmem_get_vma(pid, vma_id, vma->e->start,
			     vma_entry_len(vma->e), &fd);
	if (ret == -ENOTSUP) {
		fallback_provider_vma(vma);
		return 0;
	}
	if (ret) {
		close_safe(&fd);
...
int open_vmas(struct pstree_item *t)
{
	...
	list_for_each_entry(vma, &vmas->h, list) {
		...
		if (vma->e->status & VMA_EXT_PROVIDER) {
			...
			ret = open_provider_vma(vma, pid, current_vma_id,
						 &provider_fd_acquired);
			if (ret < 0) {
				exit_code = -1;
				goto out;
			}
