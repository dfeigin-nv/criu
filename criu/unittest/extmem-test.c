...
static void test_normal_provider(void)
{
	...
	assert(extmem_validate_memfd_mapping_fd(sealable_fd, 0x1000) == 0);
	...
	assert(regular_fd >= 0);
	assert(extmem_validate_mapping_fd(regular_fd, 1) == 0);
	assert(extmem_validate_memfd_mapping_fd(regular_fd, 1) == -1);
	assert(extmem_validate_memfd(regular_fd, 1, 0) == -1);
	close(regular_fd);
	assert(extmem_get_vma(123, 0, 0x1000, 0x2000, &image_fd) == 0);
	assert(extmem_validate_memfd_mapping_fd(image_fd, 0x2000) == 0);
	close(image_fd);
	assert(extmem_get_shared(55, 0x3000, &shared_fd) == 0);
	assert(extmem_validate_memfd_mapping_fd(shared_fd, 0x3000) == 0);
