...
usage:
	...
	pr_msg("\n"
	       ...
	       "  --stream-restore      restore using the streaming Pipeline C path\n"
	       "                        (memfd + UFFD CONTINUE; image data is fed by\n"
	       "                        an external streamer over a control socket)\n"
	       "  --stream-private MODE how private anonymous VMAs are restored under\n"
	       "                        --stream-restore: 'mmap' (default) maps them\n"
	       "                        MAP_PRIVATE over the streamer memfd (zero-copy,\n"
	       "                        CoW); 'copy' reads the same memfd into anonymous\n"
	       "                        memory eagerly; 'uffd' leaves the pages unmapped\n"
	       "                        and demand-pages them via UFFDIO_COPY. 'copy' and\n"
	       "                        'uffd' keep the VMAs genuinely anonymous at the\n"
	       "                        cost of a copy and ~2x peak RSS\n"
