...
/*
 * The pid prefix ("%6d: ") is formatted once per task in the
 * single-threaded restore setup path (log_init_by_pid) and is only read
 * afterwards. The timestamp and the message itself are built into a
 * stack-local buffer on every vprint_on_level() call. This keeps logging
 * thread-safe so the asyncd worker threads can log in parallel without
 * garbling each other's lines.
 */
