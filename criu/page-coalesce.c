...
#define COALESCE_MAX_THREADS 16
...
static void murmurhash3_x64_128(const void *key, int len, u32 seed, u64 *lo, u64 *hi)
{
	const u8 *data = key;
...
static void compute_page_hash_key(const void *page, struct page_hash_key *out)
{
	murmurhash3_x64_128(page, PAGE_SIZE, 0, &out->w0, &out->w1);
	murmurhash3_x64_128(page, PAGE_SIZE, 1, &out->w2, &out->w3);
}
...
static int read_exact_at(int fd, void *buf, size_t len, off_t off, const char *what)
{
	size_t done = 0;
	...
	while (done < len) {
		...
		if (ret < 0) {
			pr_perror("Can't read %s at offset %jd", what, (intmax_t)(off + done));
			return -1;
		}
		if (ret == 0) {
			pr_err("Unexpected EOF while reading %s at offset %jd\n", what, (intmax_t)(off + done));
			return -1;
		}
		done += ret;
	}
...
static bool page_is_all_zero(const char *page)
{
...
static int page_store_ensure_capacity(struct page_store *store)
{
	size_t new_cap;

	if (!store->cap)
		return page_store_resize(store, next_power_of_two(1 << 20));

	if (store->used * 10 < store->cap * 7)
		return 0;
	...
	return page_store_resize(store, new_cap);
}
...
static int page_store_lookup_or_reserve(struct page_store *store, const struct page_hash_key *key, u64 *offset, bool *is_new)
{
	...
	if (page_store_ensure_capacity(store))
		return -1;
...
static void *hash_worker_main(void *arg)
{
	...
	for (;;) {
		...
		while (start < end) {
			...
			batch->meta[start].zero = page_is_all_zero(page);
			if (!batch->meta[start].zero)
				compute_page_hash_key(page, &batch->meta[start].key);
			start++;
		}
...
static void hash_pages_serial(const char *pages, struct page_batch_meta *meta, unsigned int nr_pages, u64 *hash_us)
{
	...
	for (i = 0; i < nr_pages; i++) {
		...
		meta[i].zero = page_is_all_zero(page);
		if (!meta[i].zero)
			compute_page_hash_key(page, &meta[i].key);
	}
				...
				struct coalesce_stats *stats)
{
	...
	pages_buf = xmalloc(COALESCE_BATCH_PAGES * PAGE_SIZE);
	...
	if (!pages_buf || !meta || !offsets || !blob_iov)
		goto out;
	...
	while (1) {
		...
		for (i = 0; i < pe->nr_pages;) {
			...
			if (read_exact_at(old_pages_fd, pages_buf, chunk_bytes, source_off, "pages image"))
				goto out;
			image_read_us += now_us() - step_start_us;

			if (hash_pool_run(pool, pages_buf, meta, chunk_pages, &image_hash_us))
				goto out;
			...
			for (j = 0; j < chunk_pages; j++) {
				...
				if (meta[j].zero) {
					...
					if (page_store_lookup_or_reserve(store, &meta[j].key, &offsets[j], &is_new))
						goto out;
					if (is_new) {
						blob_iov[nr_blob_pages].iov_base = pages_buf + j * PAGE_SIZE;
	...
	pr_info("Image %u coalesced: present=%llu unique=%llu zero=%llu total=%llu ms read=%llu ms hash=%llu ms lookup=%llu ms blob=%llu ms index=%llu ms\n",
...
int coalesce_checkpoint_pages(void)
{
	...
	pr_info("Coalesce timings: total=%llu ms read=%llu ms hash=%llu ms lookup=%llu ms blob=%llu ms index=%llu ms grow=%llu ms workers=%u batch_pages=%u\n",
		...
		(unsigned long long)(store.grow_us / 1000ULL), pool.nr_threads, COALESCE_BATCH_PAGES);
	return 0;
}
