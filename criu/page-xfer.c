...
static int recv_full(int sk, void *buf, size_t size, const char *what)
{
	...
	while (done < size) {
		ssize_t ret = __recv(sk, (char *)buf + done, size - done,
				     MSG_WAITALL);
...
static int page_server_serve(int sk)
{
	...
	while (1) {
		...
		case PS_IOV_ADD_F_COMPRESSED: {
			...
			/*
			 * The client sends compressed records but this server
			 * writes uncompressed images: the two disagree on
			 * compression. Fail cleanly instead of storing a
			 * compressed payload the restore side would misread.
			 */
			if (!opts.compress_mode) {
				pr_err("Received compressed pages but page-server compression is off; client and server disagree on --compress\n");
				ret = -1;
				break;
			}
		...
		case PS_IOV_HOLE: {
			...
			/*
			 * This server compresses, but the client is sending
			 * uncompressed present pages. The compressed writer
			 * consumes fixed page-sized units and would stall on
			 * the non-page-aligned splice chunks this path feeds
			 * it, so reject the mismatch up front. Holes carry no
			 * payload and are unaffected.
			 */
			if (opts.compress_mode && (flags & PE_PRESENT)) {
				pr_err("Received uncompressed pages but page-server compression is on; client and server disagree on --compress\n");
				ret = -1;
				break;
			}
